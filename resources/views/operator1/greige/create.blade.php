      ...
      <div class="card-body">
        <form id="sacon_form" method="POST" action="{{ route('greige.store') }}" role="form">
            ...
            <div class="row"> 
              ...
              <div class="col-sm-10 col-10">
                <label>{{ ': '.$sacon->lot }}</label>
              </div>
            </div>
            <input type="hidden" name="item" value="{{ $sacon->item }}">
            <input type="hidden" name="id" id="idsacon" value="{{ $sacon->id }}">
            <input type="hidden" name='weaving_id' value="{{ $weaving->id }}">
            <div class="text-primary d-flex justify-content-center" ><b>Data</b></div>
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        <label>P Item</label>
                        <input type="text" id="pitem" name="pitem" class="form-control" value="{{ $weaving->pitem }}" onchange="ajaxpotong()" readonly style="text-transform: uppercase" />
                    </div>
                </div>
                <div class="col-sm-6">
                  <div class="form-group">
                      <label>Potongan</label>
                      <input type="number" id="potongan" name="potongan" value="{{ $weaving->potongan }}" class="form-control" onchange="snchange()" style="text-transform: uppercase" readonly/>
                  </div>
                </div>
                <div class="col-sm-6">
                  <div class="form-group">
                      <label>SN</label>
                      <input type="text" id="sn" name="sn" class="form-control" value="{{ $weaving->sn }}" style="text-transform: uppercase"  readonly required>
                  </div>
                </div>
                <div class="col-sm-6">
                  <div class="form-group">
...
<script type="text/javascript">
 ...
 $('#sacon_form').validate({
  ...
  function backurl(){
      ...
      window.open('/greige?kp='+kp,'_self')
