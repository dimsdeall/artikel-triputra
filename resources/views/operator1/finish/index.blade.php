...
@section('title', 'Operator1 | Finish')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#finish-table').DataTable({
                ...
                serverSide: true,
                method: 'GET',
                ajax: "{{ URL::to('/finish/operator1/getdata/' . $sacon->id) }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false,
...
@section('content')
        ...
        <div class="card card-info card-tabs">
            <div class="card-header p-0 pt-1">
                <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                    <li class="nav-item col-6">
                        <a class="nav-link active" id="custom-tabs-one-greige-tab" data-toggle="pill"
                            href="#custom-tabs-one-greige" role="tab" aria-controls="custom-tabs-one-greige"
                            aria-selected="true">Greige</a>
                    </li>
                    <li class="nav-item col-6">
                        <a class="nav-link" id="custom-tabs-one-finish-tab" data-toggle="pill"
                            href="#custom-tabs-one-finish" role="tab" aria-controls="custom-tabs-one-finish"
            ...
            <div class="card-body">
                <div class="tab-content" id="custom-tabs-one-tabContent">
                    <div class="tab-pane fade show active" id="custom-tabs-one-greige" role="tabpanel"
                    ...
                    <div class="tab-pane fade" id="custom-tabs-one-finish" role="tabpanel"
                        aria-labelledby="custom-tabs-one-finish-tab">
                        <div class="row">
                            <input type="hidden" id="kp" name="kp"
                                value="{{ $_GET['kp'] }}">
                            <div class="table-responsive">
                                <table id="finish-table" class="table table-sm table-striped table-bordered nowrap w-100">
                                    <thead>
                                        <tr>
                                            <th>
                                                no
    ...
    <form id="form-finish" action="{{ route('finish.create') }}" method="GET">
        <input type="hidden" name="kp" id="barcodeitem" value="{{ $_GET['kp'] }}">
    </form>
    ...
    <script>
        ...
        function checkbarcode() {
            ...
            if ((kp_val == '') || (id_val == null) || (id_val == '') || (status.toUpperCase() != 'GREIGE')) {
        ...
        function checkgreige() {
            console.log('test');
            let sacon_id = "{{ $sacon->id }}";
            $.ajax({
                type: 'POST',
                url: '/greige/status/check',
                data: {
                    _token: "{{ csrf_token() }}",
                    sacon: sacon_id
                },
